...
#		println("After inducer removed")
	  ...
	  T = [TP1 ; TP2 ; TP3];
	  X = [XP1 ; XP2 ; XP3];
	makePlot(T,X, "../figures/Prob3Layout5Gene2")
	return(T,X)
...
#		println("After inducer removed")
	  ...
	  T = [TP1 ; TP2 ; TP3];
	  X = [XP1 ; XP2 ; XP3];
	#makePlot(T,X, "../figures/Prob3Layout4")
	return(T,X)
	
end

function runWashoutKnockOut2(time_start, time_stop, time_step_size, data_dictionary)
	# Script to solve the balance equations -
		# First - run the model to steady-state w/no ATRA forcing -
#	println("Estimating SS")
	  XSS = estimate_steady_state(0.001,data_dictionary)
