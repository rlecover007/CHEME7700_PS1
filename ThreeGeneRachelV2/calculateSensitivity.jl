...
#	@show FIM
	...
	#A = U*diagm(S)*V'
	timestr = string("From", Int(time_start), "to", Int(time_stop))
	@show S
...
function makeHeatMap(data, timestr)
	...
	if(size(data,1)==9 && size(data,2)==9)
		...
		ax[:xaxis][:set_ticklabels](labels, rotation = 60, fontsize = 6)
		ax[:yaxis][:set_ticks](y)
		ax[:yaxis][:set_ticklabels](labels, rotation = 0, fontsize = 6)
	...
	elseif(size(data,1)==9 && size(data,2)==24)
		...
		ax[:xaxis][:set_ticks](x-.5)
		ax[:xaxis][:set_ticklabels](xlabels, rotation = 60, fontsize = 6)
		ax[:yaxis][:set_ticks](y)
		ax[:yaxis][:set_ticklabels](labels, rotation = 0, fontsize = 6)
